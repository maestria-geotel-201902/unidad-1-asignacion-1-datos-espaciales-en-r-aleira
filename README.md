...
## Reading layer `PROVCenso2010' from data source `/home/aleir/unidad-1-asignacion-1-datos-espaciales-en-r-aleira/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
plot(prov[prov$PROV %in% '10', 'PROV'])
```
...
##           areaenm2
## 1 1770605309 [m^2]
prov %>% dplyr::filter(PROV %in% '10') %>% dplyr::select("areaenm2") %>% plot
```
...
``` r
prov %>% dplyr::filter(PROV %in% c('10', '26', '29')) %>% dplyr::select("areaenm2") %>% plot
```
...
## Reading layer `MUNCenso2010' from data source `/home/aleir/unidad-1-asignacion-1-datos-espaciales-en-r-aleira/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
mimun <- mun %>% dplyr::filter(PROV %in% '10')
mimun %>% plot
```
